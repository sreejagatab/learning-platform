...
export const LearningProvider = ({ children }) => {
  ...
  const askQuestion = async (query, options = {}) => {
    ...
    try {
      ...
      const res = await api.post('/learning/query', {
        query: query,
        options: {
  ...
  const askFollowUpQuestion = async (followUpQuery) => {
    ...
    try {
      const res = await api.post('/learning/follow-up', {
        followUpQuery,
  ...
  const getUserContent = async (type = null, page = 1, limit = 10) => {
    ...
    try {
      ...
      // Check if it was a timeout or abort error
      if (err.name === 'AbortError' || err.name === 'TimeoutError') {
        throw new Error('Request timed out. Please try again later.');
      }

      // For server errors, return empty data but don't throw
      return { content: [], pagination: { total: 0 } };
    } finally {
      setContentLoading(false);
    }
  ...
  const getContentById = async (contentId) => {
    ...
    try {
      const res = await api.get(`/learning/content/${contentId}`);
      return res.data;
    } catch (err) {
      console.error('Error fetching content:', err);
      return null;
